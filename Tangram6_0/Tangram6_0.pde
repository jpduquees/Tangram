...
//Variables globales
  ...
  //Contadores: moveKey determina que ficha se esta moviendo, se maneja con el tabulador; el contadorvictoria cuenta la cantidad de pixeles blancos para determinar si el jugador gano
  ...
  //Color de cada una de las fichas y color de la pista de cada nivel, y el green determina un color verde
    color cuac = color(255,0,0), cuec  = color(0,0,204), tm1c = color(51,153,255), tm2c = color(127,0,255), tMc = color(255,255,51), tM1c  = color(0,204,102), tM2c  = color(255,128,0),blanco = color(254),green= color(15,255,46);
...
void draw(){
    ...
    if (nivel==1){
      ...
      //DELIMITACION DE AREAS PARA SELECCION DEL NIVEL Y POSICION INCIAL DE FICHA EN CADA NIVEL
        ...
        if (mouseX>=140 &&mouseX<=255 && mouseY>=155 && mouseY<=285 && mousePressed && contador==false){
            nivel=2;
            contadorNiveles=2;
          cuex= 900; cuey= 465; cuax= 770; cuay= 140; tM1x=  70; tM1y= 560; tM2x=300; tM2y= 150; tMx = 700; tMy= 335; tm1x=  35; tm1y=475; tm2x= 435; tm2y=485;
  ...
  //DIBUJOS DE SILUETAS DE CADA NIVEL
    ...
    if(nivel==11){
      ...
      background (fondo);
      fill(blanco);
      quad(203,300,413,300,308,405,203,405);
      quad(323,388,473,388,548,313,398,313);
  ...
  if (nivel !=0 && nivel !=1 && nivel !=12 && nivel !=13){
    ...
    // SISTEMA DE DETECCION DE PARTIDA GANADA
      // DETECCION DE LA CANTIDAD DE PIXELES blancoS EN LA PANTALLA
        loadPixels();
